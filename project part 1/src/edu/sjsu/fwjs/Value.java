...
class ClosureVal implements Value {
    ...
    public Value apply(List<Value> argVals) {
        // YOUR CODE HERE
    	Environment localEnv = new Environment(this.outerEnv);	//create new local environment
    	
    	for(int i = 0; i < argVals.size(); i++)
    	{
    		localEnv.createVar(params.get(i), argVals.get(i));	//bound params to matching arguments
    	}
        Value v = body.evaluate(localEnv);
